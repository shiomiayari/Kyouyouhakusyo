...
import { series1Chapters, series2Chapters, series3Chapters } from '../../data/makura-dan-data';

// Figmaアセットのインポートはビルドエラーを避けるため、静的ファイルパスに置き換えています。
// 画像ファイルは public ディレクトリに 'sei-shonagon.png' の名前で配置してください。
const seiShonagonImage = '/sei-shonagon.jpg';
  ...
  return (
    <div className="min-h-screen relative overflow-hidden pb-16">
      ...
      <section className="py-6 px-4 sm:px-6 lg:px-12">
        <div className="max-w-5xl mx-auto">
          <div className="relative bg-gradient-to-br from-[#FFE5A0] to-[#B8F3D8] p-5 md:p-8 rounded-3xl border-4 border-white">
            <div className="flex items-start gap-4">
              <div className="text-2xl md:text-3xl flex-shrink-0">✨</div>
              <div>
                <h2 className="text-base md:text-xl font-bold text-[#1B2B44] leading-relaxed">
      ...
      <section className="py-6 px-4 sm:px-6 lg:px-12">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="bg-gradient-to-br from-[#C3B7F5] to-[#FFC2D1] p-5 md:p-8 rounded-3xl border-4 border-white">
            <p className="text-sm md:text-base text-[#1B2B44] leading-relaxed mb-4">
              『枕草子』を読み進めると、きらきらした幸福感に包まれます。しかし、実は執筆当時の現実、清少納言が仕えた<strong>中宮定子の一族は政治的に没落し、定子自身も若くして亡くなるという悲劇の真っ只中</strong>でした。
            </p>
            <p className="text-sm md:text-base text-[#1B2B44] leading-relaxed">
                  ...
                  {series2Chapters.map((chapter, index) => {
                    ...
                    return (
                      <CardWrapper
                        key={`series2-${index}`}
                        {...cardProps}
                        className={`bg-[#1B2B44] p-4 rounded-2xl border-2 border-white/10 hover:border-[#FFE5A0]/50 transition-all block ${chapter.id ? 'cursor-pointer' : 'cursor-default'}`}
                      >
                        <div className="flex items-start gap-3">
                          <div className="text-2xl">💭</div>
                          <div>
                  ...
                  {series3Chapters.map((chapter, index) => {
                    ...
                    return (
                      <CardWrapper
                        key={`series3-${index}`}
                        {...cardProps}
                        className={`bg-[#1B2B44] p-4 rounded-2xl border-2 border-white/10 hover:border-[#FFC2D1]/50 transition-all block ${chapter.id ? 'cursor-pointer' : 'cursor-default'}`}
                      >
                        <div className="flex items-start gap-3">
                          <div className="text-2xl">👑</div>
                          <div>
